...
bool valid(int i, int j, int k, int n) {
	bool valid = (!((lm[i]>>n)&1)) && (!((cm[j]>>n)&1)) && (!((sm[k]>>n)&1));
	if (k >= N) valid &= ((slm[k-N][i%N]>>n)&1);
	if (k < NN-N) valid &= (((slm[k+N][i%N]>>n)&1) || (!((sm[k+N]>>n)&1)));

	if (k%N > 0) valid &= ((scm[k-1][j%N]>>n)&1);
	if (k%N < N-1) valid &= (((scm[k+1][j%N]>>n)&1) || (!((sm[k+1]>>n)&1)));
	return valid;
}
