...
bool magic_cmp(poly a, poly b) {
	if (a.size() < 3) return false;
	return inpoly_convex(b[0], a);
}
...
/*void dfs(int ind, int id) {
	vis[ind] = id;
	fr(i,ind+1,N) if (vis[i] == -1 && magic_cmp(R[ind], R[i])) dfs(i,1-id);
}*/
...
int main() {
	...
	while (scanf("%d", &N) == 1 && N > 0) {
		...
		//sort(R, R+N, magic_cmp);
		rp(i,N) rp(j,N) {
			if (i == j) continue;
			if (magic_cmp(R[i], R[j])) vis[j]++;
		}
		
		com = 0;
		res[0] = res[1] = 0;
		rp(i,N) {
			//if (vis[i] == -1) dfs(i,0), com+=(R[i].size() > 2);
			if (vis[i] == 0) com++;
			vis[i]%=2;
			M = R[i].size();
			rp(j,M) {
				res[vis[i]] += con[vis[i]]*norma(R[i][j]-R[i][(j+1)%M]);
			}
		}
